...
function breezingforms_add_admin_scripts(){
    // scripts
    wp_enqueue_script(mt_rand(0, mt_getrandmax()), WP_PLUGIN_URL . '/breezingforms/joomla-platform/media/system/js/mootools-core.js');
    wp_enqueue_script(mt_rand(0, mt_getrandmax()), WP_PLUGIN_URL . '/breezingforms/joomla-platform/media/system/js/core.js');
    wp_enqueue_script(mt_rand(0, mt_getrandmax()), WP_PLUGIN_URL . '/breezingforms/joomla-platform/media/system/js/mootools-more.js');
    wp_enqueue_script(mt_rand(0, mt_getrandmax()), WP_PLUGIN_URL . '/breezingforms/joomla-platform/media/system/js/modal.js');
    wp_enqueue_script(mt_rand(0, mt_getrandmax()), WP_PLUGIN_URL . '/breezingforms/joomla-platform/media/system/js/tabs.js');
    // styles
    
    echo '<link rel="stylesheet" href="'.WP_PLUGIN_URL . '/breezingforms/joomla-platform/media/system/css/modal.css"/>';
    
    if(isset($_REQUEST['task']) && $_REQUEST['task'] == 'editform' && $_REQUEST['act'] == 'editpage'){
        echo '<link rel="stylesheet" href="'.WP_PLUGIN_URL . '/breezingforms/joomla-platform/administrator/templates/bluestork/css/template.css"/>';
    }
}
...
function breezingforms_insert_form_button($content){
    $content .= '<a href="#TB_inline?width=450&height=550&inlineId=breezingforms_insert_form" class="thickbox" title="BreezingForms"><img src="'.WP_PLUGIN_URL.'/breezingforms/breezingforms-icon.png" alt="BreezingForms" /></a>';
    return $content;
}
...
function breezingforms_admin_init(){
    add_object_page('BreezingForms', 'BreezingForms', 'administrator', 'breezingforms', 'breezingforms_admin', BF_PLUGINS_URL . '/breezingforms/breezingforms-icon.png');
}
...
function breezingforms_site($atts = array()){
    ...
    if(isset($atts['iframe']) && $atts['iframe']){
        ...
        if(isset($atts['iframe_autoheight']) && $atts['iframe_autoheight']){
            echo '<script type="text/javascript" src="'.WP_PLUGIN_URL . '/breezingforms/joomla-platform/components/com_breezingforms/libraries/jquery/jq.iframeautoheight.js"></script>'."\n";
...
if(class_exists('WP_Widget')){
    require_once(WP_PLUGIN_DIR . '/breezingforms/BreezingFormsWidget.php');
